...
puts person.age    
```
We can also use a method with the same name as an instance variable to create a simple getter and setter. In this example, the name method serves both purposes.
```ruby
class Cat
...
puts cat.name
```
Here, the name method has the same name as the @name instance variable. When we call cat.name without providing an argument, the method returns the current value of @name, acting as a getter.

When we provide an argument, such as cat.name("Mocho"), the method updates @name with the new value, acting as a setter.
### Instance Methods and Attributes
When designing classes, we sometimes need functionality that doesn't require creating an instance of the class. Class variables, marked with double `@@` symbols, are shared across all instances of a class and can store values common to the entire class. We can access class variables using class methods.

Class methods are defined by prefixing the method name with `self.`. This allows them to be called directly on the class without creating an instance. Class methods are particularly useful for operations that don't depend on specific instance data.
```ruby
class MathUtils
...
```
### Class Constants
We can also create class constants. Unlike class variables, constants are intended to store values that should not change.

We access a class constant using the `::` operator:
```ruby
class MathUtils
...
**`private` methods** are only accessible from within the same instance of the class. We typically use private methods for internal logic that we don’t want to expose or allow other parts of the program to access directly.  
...
Another category is **runtime errors**, which occur during script execution. These can be caused by invalid user input or unpredictable conditions. 
### Types of Errors
...
If we have a block of code that we know might cause errors, we put it inside the `begin` block. Then, to catch and handle the errors, we use the `rescue` keyword followed by the error type. We can use `StandardError` to catch all the errors.  
...
# If we enter text or 0 for num2 we will have division by 0 error
```
